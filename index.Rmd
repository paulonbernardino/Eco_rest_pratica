...
[Cary Institute of Ecosystem Studies](https://www.caryinstitute.org/science/our-scientists/dr-paulo-bernardino)

[Centro de Ecologia Integrativa, UNICAMP](https://homologacaosite.com.br/cei-v3/#)
...
neededPackages <- c("raster", "sf", "gimms")
...
mask_1 <- mask
mask_1[mask_1==4] <- NA # "clouds" class
ndvi <- mask(ndvi, mask_1)
plot(ndvi)
```
...
reserve <- st_read("data/ASB_limites_expandido.shp")
...
Nesta segunda parte, vamos analisar como uma parcela que sofreu uma intervenção de restauração está evoluindo. A intervenção consistiu em remover as árvores da área degradada (que era formada por uma monocultura de uma espécies invasora), efetuar queima controlada, e então transplantar blocos de vegetação provenientes de uma área de vegetação nativa nas proximidades. Esta intervenção foi realizada 4 anos atrás.
