---
title: "Sensoriamento remoto para restauração"
...
knitr::opts_chunk$set(warning = FALSE, message = FALSE, R.options=list(width=10000), 
                      fig.align='center', out.width='100%', fig.asp=1.0) 
```
...
A máscara fornecida pelo próprio produto do Landsat separa a imagem em algumas classes: corpos d'água, estradas, vegetação e nuvens. Para esse exercício, estamos interessados em analisar a vegetação, portanto, todas as outras classes devem ser removidas da nossa imagem. Repare que as cores estão diferentes nos mapas da máscara, porém os valores relacionadas a cada classe são os mesmos.

![](figs/Figure_mask.png){ }

Neste exemplo, usaremos a máscara para remover apenas as nuvens. No exercício abaixo, o processo deve ser repetido também para as outras classes indesejadas, deixando apenas a classe de "vegetação" nas imagens que serão analizadas. Para aplicar a máscara, use o código abaixo:
...
Primeiramente, vamos avaliar as séries temporais das área de interesse. 
...
```{r, echo=TRUE, message=FALSE, out.width = 1450, out.height = 700}
...
class1 <- rep("Rest. 4 years", length(values_restoration))
class2 <- rep("Conserved", length(values_conserved))
class3 <- rep("Degraded", length(values_degraded))
...
df$class <- factor(df$class, levels = c("Rest. 4 years", "Conserved", "Degraded"))
...
(1) Processe a imagem "ndvi_mean_2022", retirando áreas com corpos d'água e estradas, recortando e excluindo áreas fora da reserva. Não é necessário mudar a escala dos dados de NDVI, eles já estão na escala correta.
...
(4) Calcule o NDVI médio do sistema de referência. Use este valor para saber, em toda a reserva, quais áreas estão mais próximas ou mais distantes desse valor 
